...
这也意味着，后续代码仓库中最好有一个 `tests/` 文件夹，至少包含：
...
其中 `test_experiments_smoke.py` 不需要一开始跑大网格，只要在 $16^2$ 或 $32^2$ 小网格上检查 M1/M2/M3 是否能完成一次短时间运行即可。这样比直接跑完整论文表格更容易定位问题。
...
| 表格型批量实验 | `experiments.py` / `test.py` | 平均迭代数和 scaling exponent |
...
如果后续要把代码整理成 git repo 并生成测试报告，可以先采用“从底层到顶层”的最小测试清单：
